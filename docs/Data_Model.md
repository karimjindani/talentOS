...
> `v0.20.9` (D-109) adds `GraduateProfile.tenantId` and `RecruiterAccessRequest.tenantId` (both
> required `String` foreign keys to `Tenant`, `onDelete: Cascade`, indexed). Migration:
> `20260824090000_v0_20_9_graduate_portal_tenant_isolation`. `GraduateProfile.tenantId` is
