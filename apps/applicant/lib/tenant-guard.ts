...
/**
 * The applicant-portal tenant-authorization gate. It binds *who you are* (the session) to *which
 * tenant you are acting on* (resolved from the Host header) via the DB TenantMembership — closing
 * the cross-subdomain leak where a shared-cookie session let a user of tenant A reach tenant B's
 * applicant portal by switching subdomains (the admin equivalent is D-051).
 *
...
export async function resolveTenantAccess(capability?: Capability): Promise<TenantAccess> {
  ...
  // Best-effort Keycloak identity backfill at login time — non-critical, never blocks the request
  // and never creates a row. Kept server-side only (out of the edge-imported auth callbacks).
  if (email) {
    try {
...
/**
 * Server-component variant that redirects on failure and returns the resolved tenant on success.
 * Unauthenticated users go to /login; unknown-tenant and non-members go to /access-denied.
 */
