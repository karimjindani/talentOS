...
import { getFullProfileForRecruiter, getRecruiterSession } from "@talentos/db";
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  ...
  const { slug } = await params; const profile = await getFullProfileForRecruiter(slug, session.recruiterId);
